...
var gaWrapper = (function () {
	function gaWrapper(opts) {
		...
		if (!opts) {
			this.log("options parameter not passed, halting", 2);return;
		}
	...
	_createClass(gaWrapper, [{
		...
		value: function register(dynamicType, match, fn) {
			if (typeof match !== "object") {
				return;
			}if (!("action" in match) && !("category" in match) && !("label" in match) || dynamicType !== "action" && dynamicType !== "category" && dynamicType !== "label") {
				return;
